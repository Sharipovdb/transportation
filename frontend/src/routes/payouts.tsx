...
function PayoutsPage() {
  ...
  const rows = useMemo<FlatPayoutRow[]>(() => {
    const sheetsInPeriod = sheets.filter(
      (sheet) => sheet.year === period.year && sheet.month === period.month,
    )

    return sheetsInPeriod.flatMap((sheet) => {
      const crewName =
        crews.find((crew) => crew.id === sheet.crewId)?.name ?? 'Unknown crew'
        ...
        <CardHeader className="flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <CardDescription className="mt-2">
              A cross-crew view of every generated sheet's computed payout
              lines. Generate and confirm sheets from the Monthly Sheets page —
                ...
                <Fragment key={row.key}>
                  <TableRow>
                    ...
                    <TableCell className="text-right">
                      {formatCurrency(row.driverPayment)}
                    </TableCell>
                    <TableCell className="text-right">
                      {formatCurrency(row.extraKmPayment)}
                    </TableCell>
                    <TableCell className="text-right">
                      {formatCurrency(row.taxiCompensation)}
                    </TableCell>
